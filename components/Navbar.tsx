...
const navItems = [
  ...
  { name: 'Insights', href: '#insights', icon: <Mail size={18} /> }, // Matches "Insights" in screenshot
];
  ...
  <>
    ...
    <nav className="fixed top-0 w-full z-50 flex justify-between items-center px-6 md:px-12 py-4 bg-white/95 backdrop-blur-sm border-b border-gray-100">
      <div className="flex items-center gap-3 group cursor-pointer">
        {/* Logo Container - Styled to match screenshot */}
        ...
        <div className="flex flex-col">
          <span className="text-lg font-serif font-bold tracking-tight text-[#0b162c] leading-none uppercase">
            MIZAN
          </span>
      ...
      {/* Desktop Navigation - Light Theme Styling */}
      <div className="hidden md:flex gap-10 text-[13px] font-medium text-[#0b162c]/70">
        {navItems.map((item) => (
          <motion.a 
            key={item.name} 
            href={item.href}
            whileHover={{ color: '#0b162c' }}
            className="transition-colors cursor-pointer hover:border-b hover:border-[#c5a358] pb-1"
          >
            {item.name}
          </motion.a>
        ))}
      ...
      {/* CTA Button - Navy/Gold theme from image */}
      <motion.button 
        whileHover={{ backgroundColor: '#162b4d' }}
        whileTap={{ scale: 0.97 }}
        className="bg-[#0b162c] text-white px-6 py-2.5 rounded-full font-bold text-xs flex items-center gap-2 shadow-md transition-all"
      >
      ...
      <div className="flex justify-around items-center h-16 px-4">
        {navItems.map((item) => (
          <motion.a
            key={item.name}
            href={item.href}
            className="flex flex-col items-center justify-center gap-1 text-[#0b162c]/60"
            whileTap={{ scale: 0.9 }}
          >
